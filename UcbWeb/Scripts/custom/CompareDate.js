...
function compareDate(value, element, params) {
    ...
    var sdatevalueukdate = new Date(sdateukarray[2], parseInt(sdateukarray[1]) - 1, sdateukarray[0]);

    var dateukarrayukdate = value.split('/');
    if (dateukarrayukdate.length != 3) return true;
    
    var valuedateuk = new Date(dateukarrayukdate[2], parseInt(dateukarrayukdate[1]) - 1, dateukarrayukdate[0]);
